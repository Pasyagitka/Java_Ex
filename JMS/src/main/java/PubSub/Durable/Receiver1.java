...
public class Receiver1 implements MessageListener {
    ...
    Receiver1() {
        try (JMSContext context = factory.createContext("admin", "admin", JMSContext.AUTO_ACKNOWLEDGE )) {
            ...
            context.setClientID("durableConsumer");
            Destination priceInfo= context.createTopic("PubSub");
            consumer = context.createDurableConsumer((Topic) priceInfo, "SecurityCenter");
    ...
    public void onMessage(Message message) {
        try {
            System.out.println("Got message priority: "+ message.getBody(String.class));
        } catch (Exception e) {
